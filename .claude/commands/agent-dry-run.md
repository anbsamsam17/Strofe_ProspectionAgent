...
- `OPENAI_API_KEY` (gpt-4o, pour les pitchs)
...
Lance `/sync-env` si tu doutes. Si une clé essentielle (OpenAI, Supabase, INSEE) manque : arrête-toi.
...
2. Passe la variable d'env `DRY_RUN=true` au script et logge dans `lib/agent/orchestrator.ts` un `if (process.env.DRY_RUN === 'true')` qui court-circuite les `updateRunInDB`, `INSERT prospects`, `INSERT daily_list_items` et l'envoi Resend.
...
2. Observe les logs JSON structurés sur stdout. Chaque phase apparaît avec son timestamp :
   ...
   - `phase: 'scoring'` — distribution des scores (min/max/médiane).
   - `phase: 'selection'` — les 15 top selon `score_priorite` desc.
   - `phase: 'enrichment_contact'` — taux de succès Recherche Entreprises / Pappers / Hunter.
   - `phase: 'pitch_gen'` — 15 pitchs OpenAI gpt-4o générés, format JSON valide.
3. Sauvegarde la sortie : `npx tsx scripts/agent-dry-run.ts <user_id> > dry-run-$(date +%s).log`.

## 4. Vérifications post-run

1. Aucun crash, aucune erreur OpenAI rate limit (le batch a `BATCH_DELAY_MS=150` et `PARALLEL_GROUP_SIZE=5` dans `lib/agent/pitch-gen.ts`).
2. Les 15 pitchs ont la structure attendue (cf. `GeneratedPitch` dans `lib/types.ts`) : `accroche`, `pitch`, `signaux_detectes[]`, `objections[]`, `meilleur_creneau`, `contact_type`, `ton`.
3. L'ordre rhétorique des pitchs respecte ROI → image → légal (relis le SYSTEM_PROMPT dans `lib/agent/pitch-gen.ts:47-79`).
